...
export default function PinModal({ mode, adminPinHash, onSuccess, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "verify") {
      ...
      if (hashPin(pin) === adminPinHash) { onSuccess(); }
      else setError("PIN incorrecto");
      setLoading(false);
      return;
    }

    if (newPin.length < 4) { setError("El PIN debe tener al menos 4 caracteres"); setLoading(false); return; }
    ...
    if (res.ok) { onSuccess(); }
    else { const d = await res.json(); setError(d.error || "Error al cambiar el PIN"); }
    setLoading(false);
  }

  return (
    <div className="fixed inset-0 bg-tw-navy/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-tw-grey/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "set" && adminPinHash && (
            <div>
              <label className="block text-sm font-semibold text-tw-navy/60 mb-1.5">PIN actual</label>
          ...
          {mode === "verify" && (
            <div>
              <label className="block text-sm font-semibold text-tw-navy/60 mb-1.5">PIN</label>
              <input type="password" value={pin} onChange={e => setPin(e.target.value)}
                className="w-full border-2 border-tw-grey/40 rounded-xl px-4 py-3 text-base focus:outline-none focus:border-tw-green"
          ...
          {mode === "set" && (
            <div>
              <label className="block text-sm font-semibold text-tw-navy/60 mb-1.5">Nuevo PIN</label>
