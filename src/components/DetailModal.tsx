...
export default function DetailModal({ p, results, onClose }: Props) {
  ...
  const tabs: [DTab, string][] = [
    ["resumen", "Resumen"], ["grupos", "Grupos"], ["espana", "🇪🇸 España"],
    ["elim", "Eliminatoria"], ["bonus", "Bonus"],
  ];
    ...
    <div className="fixed inset-0 bg-tw-navy/70 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh] border-2 border-tw-grey/10">
        ...
        <div className="flex items-center justify-between px-5 sm:px-6 py-4 sm:py-5 bg-tw-navy sm:rounded-t-2xl">
          <div>
            <p className="text-white/70 text-sm font-medium mb-0.5">{p.nombre}</p>
            <p className="text-4xl sm:text-5xl font-extrabold text-tw-green leading-none">{score.total} <span className="text-xl text-tw-green/70 font-semibold">pts</span></p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 bg-tw-light sm:rounded-b-2xl">
          {tab === "resumen" && (
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {[
                { label: "P1 — 1X2",         pts: score.p1, sub: `${bd.p1ok}/${bd.p1n} acertados` },
                { label: "P2 — 🇪🇸 España",  pts: score.p2, sub: `${bd.p2detail.filter(d => d?.label === "exacto").length}/3 exactos` },
                { label: "P3 — Eliminatoria", pts: score.p3, sub: `${bd.champ ? "✅" : "❌"} campeón` },
                { label: "P4 — Bonus",        pts: score.p4, sub: `${bd.b.filter(Boolean).length}/4 aciertos` },
              ].map(({ label, pts, sub }) => (
          ...
          {tab === "grupos" && (
            <div className="space-y-5">
              {GROUPS.map(g => {
                ...
                return (
                  <div key={g}>
                    <h4 className="font-bold text-sm sm:text-base text-tw-navy/60 mb-2 uppercase tracking-wide">Grupo {g}</h4>
          ...
          {tab === "espana" && (
            <div className="space-y-4">
              <div className="bg-tw-navy/5 rounded-xl p-3 text-sm text-tw-navy/70 font-medium">
                Marcador exacto = <strong className="text-tw-navy">10 pts</strong> · Cualquier otro resultado = <strong className="text-tw-navy">0 pts</strong>
              </div>
              {SPAIN_IDX.map((mi, k) => {
                const m = MATCHES[mi];
                ...
                return (
                  <div key={mi} className={`p-4 sm:p-5 rounded-2xl border-2 ${d?.label === "exacto" ? "bg-tw-green/10 border-tw-green/40" : s ? "bg-red-50 border-red-200" : "bg-white border-tw-grey/20"}`}>
                    <div className="flex justify-between items-start mb-3 gap-2">
                      ...
                      <span className={`text-base sm:text-lg font-extrabold shrink-0 ${d?.pts ? "text-tw-navy" : "text-tw-grey"}`}>+{d?.pts ?? 0} pts</span>
                    </div>
                    <div className="flex gap-4 sm:gap-6 text-sm">
                      <span><span className="text-tw-grey">Marcador: </span><strong className="font-mono text-base text-tw-navy">{p.picks.p2[k] || "—"}</strong></span>
                      {s && <span><span className="text-tw-grey">Real: </span><strong className="font-mono text-base text-tw-navy">{s.h}-{s.a}</strong></span>}
                    </div>
                    {d && <div className="mt-3">
                      <span className={`text-sm px-3 py-1 rounded-full font-semibold ${d.label === "exacto" ? "bg-tw-green text-tw-navy" : "bg-red-100 text-red-600"}`}>{d.label}</span>
                    </div>}
                  </div>
                );
              })}
              ...
              {[
                ...
                <div key={label} className={`p-4 rounded-2xl border-2 ${hit ? "bg-tw-green/10 border-tw-green/40" : actual ? "bg-red-50 border-red-200" : "bg-white border-tw-grey/20"}`}>
                  ...
                  <div className="flex gap-6 text-sm sm:text-base">
                    <span><span className="text-tw-grey">Pred: </span><strong className="text-tw-navy">{pred || "—"}</strong></span>
                    {actual && <span><span className="text-tw-grey">Real: </span><strong className="text-tw-navy">{actual}</strong></span>}
                  </div>
                </div>
              ))}
              {[
                { label: "Semifinalistas (15 pts c/u)",  pred: p.picks.p3.semis, actual: results.knockout.semis, sc: bd.sem },
                { label: "Cuartos de final (6 pts c/u)", pred: p.picks.p3.qf,    actual: results.knockout.qf,    sc: bd.qf  },
                { label: "Ronda de 16 (3 pts c/u)",      pred: p.picks.p3.r16,   actual: results.knockout.r16,   sc: bd.r16 },
                { label: "Ronda de 32 (2 pts c/u)",      pred: p.picks.p3.r32,   actual: results.knockout.r32,   sc: bd.r32 },
              ].map(({ label, pred, actual, sc }) => (
                <div key={label}>
                  <div className="text-xs sm:text-sm text-tw-grey font-semibold mb-2 flex justify-between">
                    <span>{label}</span>
                    <strong className="text-tw-navy">{sc.pts} pts ({sc.hits} aciertos)</strong>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {pred.map((t, j) => {
                      const hit = actual.some(a => norm(a) === norm(t));
                      return <span key={j} className={`text-xs sm:text-sm px-3 py-1.5 rounded-xl font-semibold ${hit ? "bg-tw-navy text-tw-green" : "bg-white text-tw-navy border border-tw-grey/30"}`}>{t || "—"}</span>;
                    })}
                  </div>
                </div>
              ))}
          ...
          {tab === "bonus" && (
            <div className="space-y-3">
              {[
                ...
                { label: "🇪🇸 Máx. goleador ESP",  pred: p.picks.p4.topEspScorer,  hit: bd.b[3] },
              ].map(({ label, pred, hit }) => (
                <div key={label} className={`flex justify-between items-center p-4 sm:p-5 rounded-2xl border-2 ${hit ? "bg-tw-green/10 border-tw-green/40" : "bg-white border-tw-grey/20"}`}>
                  ...
                  <span className={`text-base sm:text-lg font-extrabold ${hit ? "text-tw-navy" : "text-tw-grey"}`}>
                    {hit ? "+10 pts" : "0 pts"}
                  </span>
                </div>
              ))}
