...
interface Status { ok: boolean; reason?: string; competition?: string; rpmAvailable?: number | null; }
...
export default function ApiStatus() {
  ...
  useEffect(() => {
    fetch("/api/status")
      ...
      .catch(() => { setStatus({ ok: false, reason: "Error" }); setLoading(false); });
  }, []);
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-xs sm:text-sm font-medium ${status?.ok ? "text-tw-green" : "text-red-400"}`}>
        {status?.ok
          ? t.apiOnline(status.rpmAvailable ?? null)
