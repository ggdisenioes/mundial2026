  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-tw-navy">Resultados del torneo</h2>
          {lastUpdate && <p className="text-sm text-tw-grey mt-1">Última actualización: {lastUpdate}</p>}
        </div>
        <div className="flex gap-2">
          {unlocked ? (
            <>
              <button onClick={() => setShowChangePin(true)}
                className="text-sm sm:text-base border-2 border-tw-grey/40 text-tw-navy px-4 py-2.5 rounded-xl font-semibold hover:border-tw-navy transition-colors">
                🔑 Cambiar PIN
              </button>
            ...
            <button onClick={() => settings.adminPinHash ? setShowPinModal(true) : setUnlocked(true)}
              className="text-sm sm:text-base bg-tw-green text-tw-navy px-5 py-2.5 rounded-xl font-bold hover:opacity-90 transition-opacity shadow-sm">
              ✏️ Editar resultados
            </button>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl border-2 border-tw-grey/20 shadow-sm p-4 sm:p-6">
        <h3 className="font-bold text-lg sm:text-xl text-tw-navy mb-4">Resultados de grupos</h3>
          ...
          {MATCHES.map(([h, a, g], i) => (
            <div key={i} className="flex items-center gap-2 bg-tw-light rounded-xl px-3 py-2.5">
              ...
              {unlocked ? (
                ...
                <span className={`w-16 sm:w-20 text-center font-mono font-bold text-base px-2 py-1.5 rounded-lg ${scores[i] ? "bg-tw-navy text-tw-green" : "text-tw-grey"}`}>
                  {scores[i] || "—"}
                </span>
              )}
      ...
      <div className="bg-white rounded-2xl border-2 border-tw-grey/20 shadow-sm p-4 sm:p-6 space-y-5">
        <h3 className="font-bold text-lg sm:text-xl text-tw-navy">Fase eliminatoria</h3>
          ...
          {[
            ...
            <div key={key}>
              ...
              {unlocked
                ...
                : <div className={readCls}>{knockout[key] || <span className="text-tw-grey">Sin definir</span>}</div>
              }
            </div>
          ))}
        </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {Array.from({ length: n }, (_, j) => (
                  <select key={j} value={knockout[key][j] || ""}
                    ...
                    className="border-2 border-tw-grey/40 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-tw-green bg-white">
                    <option value="">—</option>
                    {TEAM_LIST.map(t => <option key={t}>{t}</option>)}
                  </select>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {knockout[key].filter(Boolean).length > 0
                  ? knockout[key].filter(Boolean).map((t, j) => (
                    <span key={j} className="text-sm sm:text-base px-3 py-1.5 bg-tw-navy text-tw-green rounded-xl font-semibold">{t}</span>
                  ))
                  : <span className="text-sm text-tw-grey">Sin definir</span>
                }
              </div>
            )}
          </div>
      ...
      <div className="bg-white rounded-2xl border-2 border-tw-grey/20 shadow-sm p-4 sm:p-6 space-y-4">
        <h3 className="font-bold text-lg sm:text-xl text-tw-navy">Bonus</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            ...
            <div key={key}>
              ...
              {unlocked ? (
                type === "text"
                  ? <input value={bonus[key]} onChange={e => setBonus({ ...bonus, [key]: e.target.value })} className={inputCls} placeholder="Dejar vacío = automático" />
                  : <select value={bonus[key]} onChange={e => setBonus({ ...bonus, [key]: e.target.value })} className={inputCls}>
                      <option value="">— Automático —</option>
                      {TEAM_LIST.map(t => <option key={t}>{t}</option>)}
                    </select>
              ) : (
                <div className={readCls}>{bonus[key] || <span className="text-tw-grey">Automático</span>}</div>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {settings.adminPinHash && (
            <div className="bg-white rounded-2xl border-2 border-tw-grey/20 p-4 sm:p-6">
              <label className="block text-sm font-semibold text-tw-navy/60 mb-2">PIN para confirmar</label>
              <input type="password" value={pin} onChange={e => setPin(e.target.value)} className={inputCls} placeholder="Ingresá el PIN" />
            </div>
          )}
